...
output_dir="$PKG_DB/V_TAGS" # this dir contains files for every repo with its installed packages_tag also to compare with P_TAGS folders.
mkdir -p "$output_dir"
...
for package in /var/lib/pkgtools/packages/*; do
    ...
    if [ "$num_parts" -ge 4 ]; then
        ...
        version="${parts[1]}"
        arch="${parts[2]}"
        build_version="1"  # Default build version if not provided. we need this for flatpaks ;)
